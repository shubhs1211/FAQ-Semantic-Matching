...
def clean(tokens):
	...
	pos_tagged_tokens = tokens

	for i in range(len(tokens)):
		tokens[i] = [word for word in tokens[i] if word not in sw]
		lemmatized_tokens[i] = [wnl.lemmatize(word) for word in tokens[i]]
		stemmed_tokens[i] = [porter.stem(word) for word in tokens[i]]
		pos_tagged_tokens[i] = pos_tag(tokens[i])

	return tokens, lemmatized_tokens, stemmed_tokens, pos_tagged_tokens
...
if __name__ == '__main__':
	...
	clean_questions_list, lemmatized_questions_list, stemmed_questions_list, pos_tagged_questions_list = clean(questions_list)
	clean_answers_list, lemmatized_answers_list, stemmed_answers_list, pos_tagged_answers_list = clean(answers_list)
	clean_qa_bag, lemmatized_qa_bag, stemmed_qa_bag, pos_tagged_qa_bag = clean(qa_bag)

	print(pos_tagged_questions_list)
